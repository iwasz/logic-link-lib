...
TEST_CASE ("flexio", "[rearrange]")
{
        SECTION ("8 channels 1 shifter per channel")
        {
                ...
                REQUIRE (std::get<data::Bytes> (sd.at (0).buffer) == Bytes{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07});
                REQUIRE (std::get<data::Bytes> (sd.at (1).buffer) == Bytes{0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17});
                REQUIRE (std::get<data::Bytes> (sd.at (2).buffer) == Bytes{0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27});
                REQUIRE (std::get<data::Bytes> (sd.at (3).buffer) == Bytes{0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37});
                REQUIRE (std::get<data::Bytes> (sd.at (4).buffer) == Bytes{0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47});
                REQUIRE (std::get<data::Bytes> (sd.at (5).buffer) == Bytes{0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57});
                REQUIRE (std::get<data::Bytes> (sd.at (6).buffer) == Bytes{0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67});
                REQUIRE (std::get<data::Bytes> (sd.at (7).buffer) == Bytes{0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77});
        }
