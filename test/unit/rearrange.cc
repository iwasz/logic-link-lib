...
TEST_CASE ("flexio", "[rearrange]")
{
        SECTION ("8 channels 1 shifter per channel")
        {
                ...
                REQUIRE (sd.digital.at (0) == Buffer{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07});
                REQUIRE (sd.digital.at (1) == Buffer{0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17});
                REQUIRE (sd.digital.at (2) == Buffer{0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27});
                REQUIRE (sd.digital.at (3) == Buffer{0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37});
                REQUIRE (sd.digital.at (4) == Buffer{0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47});
                REQUIRE (sd.digital.at (5) == Buffer{0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57});
                REQUIRE (sd.digital.at (6) == Buffer{0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67});
                REQUIRE (sd.digital.at (7) == Buffer{0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77});
        }
        ...
        SECTION ("8 channels 1 shifter per channel")
        {
                ...
                REQUIRE (sd.digital.at (0)
                         == Buffer{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f});
                REQUIRE (sd.digital.at (1)
                         == Buffer{0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f});
                REQUIRE (sd.digital.at (2)
                         == Buffer{0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f});
                REQUIRE (sd.digital.at (3)
                         == Buffer{0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f});
        }
        ...
        SECTION ("1 channel 2 shifters")
        {
                ...
                SampleData sd = an::rearrangeFlexio<1, 2> (raw);
        ...
        SECTION ("1 channel 8 shifters")
        {
                ...
                SampleData sd = an::rearrangeFlexio<1, 8> (raw);
