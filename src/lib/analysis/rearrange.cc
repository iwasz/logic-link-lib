...
SampleData rearrangeFlexio (RawData const &rd, common::acq::Params const &params);
SampleData rearrangeGpio1_2 (RawData const &rd, common::acq::Params const &params);

/**
 * Rearrange the device speciffic data format into SampleData
 */
SampleData rearrange (RawData const &rd, common::acq::Params const &params)
{
 ...
 *                                                              D C B A
 */
SampleData rearrangeFlexio1a (RawData const &rd)
{
        ...
        using Batch = std::array<uint8_t, BYTES_PER_BATCH>;
        auto outI = sd.digital.at (0).begin ();
        ...
        for (size_t i = 0; i < rd.buffer.size (); i += BYTES_PER_BATCH) {
                ...
                for (size_t k = 0; k < (CHAR_BIT / 2); ++k) {
                        ...
                        // Column A
                        out[k * SHIFTBUFS_PER_CH_NUM + 0] = (in[12 + n] & mask1) << 7 | (in[8 + n] & mask1) << 6 | (in[4 + n] & mask1) << 5
                                | (in[0 + n] & mask1) << 4 | (in[12 + n] & mask2) << 2 | (in[8 + n] & mask2) << 1 | (in[4 + n] & mask2) << 0
                                | (in[0 + n] & mask2) >> 1;

                        // Column B
                        mask1 = 1 << 2;
                        mask2 = 1 << 3;
                        out[k * SHIFTBUFS_PER_CH_NUM + 1] = (in[12 + n] & mask1) << 5 | (in[8 + n] & mask1) << 4 | (in[4 + n] & mask1) << 3
                                | (in[0 + n] & mask1) << 2 | (in[12 + n] & mask2) | (in[8 + n] & mask2) >> 1 | (in[4 + n] & mask2) >> 2
                                | (in[0 + n] & mask2) >> 3;

                        // Column C
                        mask1 = 1 << 4;
                        mask2 = 1 << 5;
                        out[k * SHIFTBUFS_PER_CH_NUM + 2] = (in[12 + n] & mask1) << 3 | (in[8 + n] & mask1) << 2 | (in[4 + n] & mask1) << 1
                                | (in[0 + n] & mask1) << 0 | (in[12 + n] & mask2) >> 2 | (in[8 + n] & mask2) >> 3 | (in[4 + n] & mask2) >> 4
                                | (in[0 + n] & mask2) >> 5;

                        // Column D
                        mask1 = 1 << 6;
                        mask2 = 1 << 7;
                        out[k * SHIFTBUFS_PER_CH_NUM + 3] = (in[12 + n] & mask1) << 1 | (in[8 + n] & mask1) >> 0 | (in[4 + n] & mask1) >> 1
                                | (in[0 + n] & mask1) >> 2 | (in[12 + n] & mask2) >> 4 | (in[8 + n] & mask2) >> 5 | (in[4 + n] & mask2) >> 6
                                | (in[0 + n] & mask2) >> 7;
                }
 ...
 */
SampleData rearrangeFlexio1b (RawData const &rd)
{
        constexpr size_t CHANNELS_NUM = 1;
        constexpr size_t SHIFTBUFS_PER_CH_NUM = 4;
        ...
        for (size_t i = 0; i < rd.buffer.size (); i += BYTES_PER_BATCH) {
                ...
                for (size_t k = 0; k < (CHAR_BIT / 2); ++k) {
                        for (size_t j = 0; j < CHAR_BIT; j += 2) {
                                ...
                                Batch n2{uint8_t (in[12 + n] << (sh - 1)), uint8_t (in[8 + n] << (sh - 1)), uint8_t (in[4 + n] << (sh - 1)),
                                         uint8_t (in[0 + n] << (sh - 1))};
...
inline SampleData rearrangeFlexio (RawData const &rd, common::acq::Params const &params)
{
...
SampleData rearrangeGpio1_2 (RawData const &rd, common::acq::Params const &params) { return {}; }
