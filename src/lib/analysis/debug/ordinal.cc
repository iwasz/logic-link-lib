...
void OrdinalCheck::run (data::SampleData const &samples)
{
        ...
        for (size_t i = 0; i < dmaBlocksNum; ++i) {
                ...
                bufferNo = *reinterpret_cast<uint32_t const *> (samples.digital.data () + i * dmaBlockLenB ());
                queueSize = *reinterpret_cast<uint32_t const *> (samples.digital.data () + i * dmaBlockLenB () + 4);

                if (lastBufferNo) {
                        if (auto diff = (int (bufferNo) - int (*lastBufferNo)); diff != 1) {
                                // std::println ("{}. bufferNo: {}, bps: {:.2f}, qSize: {}, txSize: {}, \u001b[31movr {}\u001b[0m", devBufferNo,
                                //               bufferNo, 0U /* bmd.bps */, queueSize, samples.digital.size (), diff);
